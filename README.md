...
To limit the bytes read from each ignore file, use `NewWithOptions` or `NewFromDirectoryWithOptions`:

```go
opts := gitignore.Options{MaxIgnoreFileSize: 1 << 20}
m := gitignore.NewFromDirectoryWithOptions("/path/to/repo", opts)
```
